...
# تنظيف أي كود صدارة قديم متضارب
html = re.sub(r'<script>\s*function openAccountModal\(\).*?</script>', '', html, flags=re.DOTALL)
html = re.sub(r'<!-- LEADERBOARD MODAL -->.*?</script>', '', html, flags=re.DOTALL)

# كود نقي ومستقل بالكامل بدون أي تداخل
pure_code = '''
<!-- ACCOUNT MODAL -->
...
if "</body>" in html:
    html = html.replace("</body>", pure_code + "\n</body>")
    with open("index.html", "w") as f:
        f.write(html)
    print("Pure HTML injected successfully!")
