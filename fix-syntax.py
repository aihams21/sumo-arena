...
# تنظيف شامل لأي مودل قديم لليدربورد لتبدأ نظيفة
import re
html = re.sub(r'<!-- نافذة لوحة الصدارة النهائية والآمنة.*?</script>', '', html, flags=re.DOTALL)
html = re.sub(r'<div id="leaderboard-modal".*?</div>\s*</div>\s*</div>', '', html, flags=re.DOTALL, count=1)

final_modal_system = '''
<!-- نافذة لوحة الصدارة النهائية والآمنة -->
<div id="leaderboard-modal" style="display: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.9); z-index: 99999; justify-content: center; align-items: center; font-family: 'Orbitron', sans-serif; backdrop-filter: blur(15px);">
...
html = re.sub(r'onclick="openLeaderboardModalSafe\(\)Safe\(\)"', 'onclick="openLeaderboardModalSafe()"', html)

if "</body>" in html:
    html = html.replace("</body>", final_modal_system + "\n</body>")
    with open("index.html", "w") as f:
        f.write(html)
    print("Syntax fixed and modal injected successfully!")
