...
# 1. تنظيف أي تداخل قديم للمودلات بالكامل
html = re.sub(r'<!-- نافذة لوحة الصدارة المستقلة بالكامل.*?</script>', '', html, flags=re.DOTALL)
...
# 2. حقن النظامين المنفصلين والنظيفين 100%
pure_system_code = '''
<!-- 1. نافذة تعديل الحساب (الاسم، الصورة، وعلم البلد فقط) -->
...
<!-- 2. نافذة لوحة الصدارة المستقلة تماماً -->
...
<script>
// فتح نافذة تعديل الحساب بشكل مستقل
...
// فتح نافذة لوحة الصدارة بشكل مستقل تماماً
...
if "</body>" in html:
    html = html.replace("</body>", pure_system_code + "\n</body>")
    with open("index.html", "w") as f:
        f.write(html)
    print("Pure separation system injected successfully!")
