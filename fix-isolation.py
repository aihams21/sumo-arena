...
import re
# إزالة أي تداخل قديم للمودلات
html = re.sub(r'<!-- نافذة لوحة الصدارة النهائية والآمنة.*?</script>', '', html, flags=re.DOTALL)

clean_separated_system = '''
<!-- نافذة لوحة الصدارة المستقلة بالكامل -->
...
function openLeaderboardModalSafe() {
  // إغلاق المنيو الجانبي فوراً وعدم المساس بأي مودل تاني
  const sidebar = document.getElementById('neon-sidebar');
  if (sidebar) sidebar.style.right = '-320px';
  
  // إغلاق مودل تعديل الحساب احتياطياً لعدم التداخل
...
if "</body>" in html:
    html = html.replace("</body>", clean_separated_system + "\n</body>")
    with open("index.html", "w") as f:
        f.write(html)
    print("Isolated leaderboard system injected!")
